...
def make_weighted_sampler(dataset, num_classes=23):
    ...
    Uses the 'class_id' field of the dataset (corresponding to level_3_id, ranging from 0 to 22).
    """
...
def get_expm_folder(script_path, output_folder='out', expm_id=''):
    """
    Generates the experiment folder path based on the script name and current date.
    """
...
class DistanceDict(dict):
    ...
    def __setitem__(self, key, value):
        raise NotImplementedError("Definir itens não é suportado em DistanceDict.")
...
def get_weighting(hierarchy: Tree, weighting="uniform", **kwargs):
    ...
    if weighting == "exponential":
        ...
        raise ValueError("Tipo de weighting não suportado: {}".format(weighting))
...
def accuracy(output, target, ks=(1,)):
    ...
    with torch.no_grad():
        ...
        _, pred_ = output.topk(maxk, 1, True, True)  # pred_ tem shape (batch_size, maxk)
        pred = pred_.t()  # transposta: (maxk, batch_size)
...
def get_order_family_target(targets):
    ...
    for i in range(targets.size(0)):
        ...
        if -1 in (order, family, species):
            raise ValueError(f"Target {targets[i]} possui -1 em algum nível: {[order, family, species]}")
...
class CustomEfficientNetV2(nn.Module):
    """
    CustomEfficientNetV2 model with modified head.

    Uses the pre-trained 'tf_efficientnetv2_s.in21k_ft_in1k' model from timm with the head removed.
    ...
    def __init__(self, num_classes, input_size=(384, 384)):
        ...
        """
        super().__init__()
        # Create the backbone model with the head removed.
        ...
        # Automatically detect the feature dimension from a dummy forward pass.

        device = next(self.backbone.parameters()).device  
        dummy = torch.randn(1, 3, *input_size, device=device)
        with torch.no_grad():
            features = self.backbone.forward_features(dummy)
            pooled = self.pool(features)  # Expected [1, C, 1, 1]
...
def init_model_on_gpu():
    """
    Initializes the CustomEfficientNetV2 model on the available device.
    """
    print("=> Using EfficientNetV2 with custom head")
    model = CustomEfficientNetV2(num_classes=23)
...
class HierarchicalLLLoss(nn.Module):
    """
    Hierarchical Log Likelihood Loss based on an nltk tree to define weights.
    """
    def __init__(self, hierarchy: Tree, classes: List[str], weights: Tree):
        ...
        print("Posições das folhas:", list(positions_leaves.keys()))
    ...
    def forward(self, inputs, target):
        """
        Realiza o forward pass calculando a loss.
        """
...
# =============================================================================
# DATASET AND AUGMENTATIONS DEFINITION
# =============================================================================
class InputDataset(Dataset):
    ...
    def __init__(self, data_csv_file, train=True, transform=None,
        ...
        """
        Args:
            data_csv_file: CSV contendo [image_path, class_id].
            train: Flag se o dataset é para treinamento.
            transform: Transformações do torchvision.
            albu_transform: Pipeline de augmentação com albumentations.
            rare_albu_transform: Pipeline especial para classes raras.
            rare_prob: Dicionário com probabilidade de augmentação extra.
        """
        self.train = train
    ...
    def __getitem__(self, index):
        ...
        if img is None:
            print(f"Warning: não foi possível ler {img_path}. Retornando imagem em branco.")
...
def detect_rare_classes(dataset, num_classes=23, threshold=200):
    ...
    print("Frequência das classes (0 a {}):".format(num_classes - 1), freq)
    print("Classes raras detectadas (freq < {}): {}".format(threshold, rare))
    return rare

def compute_rare_probabilities(dataset, num_classes=23, alpha=0.5):
    """
    Computes probabilities for rare classes.
    """
...
def get_order_family_target(targets):
    """
    Separates targets into three levels: order, family, and species.
    """
    order_target_list = []
    family_target_list = []
    species_target_list = []
...
def create_optimizer(model, lr_mult=1.0):
    """
    Creates an optimizer with different learning rates for the model's components.
    """
    params = [
...
# =============================================================================
# CHECKPOINTS AND AUXILIARY EVALUATION FUNCTIONS
# =============================================================================
def _load_checkpoint(model, optimizer, scheduler):
    """
    Loads a checkpoint for the model, optimizer, and scheduler.
    """
    ...
    if os.path.isfile(checkpoint_path):
        print("=> Carregando checkpoint '{}'".format("output"))
        ...
        try:
            ...
            print(f"=> Erro ao carregar optimizer: {e}. Optimizer reiniciado.")
        
        # Aplicar políticas de descongelamento do checkpoint
        print("Aplicando políticas de descongelamento até a época", start_epoch)
        for epoch_key in sorted(unfreeze_schedule.keys()):
            if epoch_key <= start_epoch:
                ...
                print(f"✅ Aplicado descongelamento da época {epoch_key}: {schedule['components']}")
        
        # Recriar optimizer e scheduler com os parâmetros do checkpoint
        optimizer = create_optimizer(model, lr_mult=current_lr_mult)
        if "optimizer" in checkpoint:
            try:
                optimizer.load_state_dict(checkpoint["optimizer"], strict=False)
            except:
                print("Não foi possível carregar o optimizer, inicializando do zero.")
...
def load_data(train_csv, val_csv, distributed):
    ...
    """
    print("Carregando dados e definindo transformações...")

    # Normalização padrão do ImageNet
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])

    # Transformações do torchvision: ToTensor e Normalize
    ...
    # Pipeline de augmentação com albumentations para treinamento
    train_albu_transform = albu.Compose([
        # Usando a nova API: RandomResizedCrop espera o parâmetro "size"
    ...
    # Pipeline agressiva para classes raras
    ...
    # Pipeline de augmentação para validação: apenas resize
...
def main_worker():
    """
    Coordena o treinamento e a validação do modelo.
    """
    ...
    # Se não houve checkpoint, usa o schedule padrão
    if not unfreeze_schedule:
        unfreeze_schedule = {
        10: {"components": ["blocks.5"], "lr_mult": 0.2},
        20: {"components": ["blocks.4"], "lr_mult": 0.3},
        30: {"components": ["blocks.3"], "lr_mult": 0.4},
        40: {"components": ["blocks.2"], "lr_mult": 0.5},
        50: {"components": ["blocks.1"], "lr_mult": 0.6},
        60: {"components": ["blocks.0"], "lr_mult": 0.7},
        70: {"components": ["conv_stem"], "lr_mult": 0.8}
    }
    ...
    for epoch in range(start_epoch, 100):  # Único loop de épocas
        json_name = f"epoch.{epoch:04d}.json"  # Definir nome do JSON
        do_validate = (epoch % 1 == 0)  # Validar a cada época

        # Aplicar política de descongelamento progressivo
...
if __name__ == "__main__":
    ...
    out_folder = "output"
    print("Garantindo que todas as pastas de output existem...")
    ...
    while retry_count < max_retries:
        try:
            print(f"\n[INFO] Iniciando tentativa {retry_count + 1} de execução...\n")
            main_worker()
            break  # Sucesso: sai do loop
        except RuntimeError as e:
            if "UR_RESULT_ERROR_OUT_OF_RESOURCES" in str(e):
                retry_count += 1
                print(f"[WARNING] Recursos insuficientes no hpu. Reiniciando tentativa {retry_count}/{max_retries}...\n")

                # Libera cache da GPU e coleta lixo
                try:
                    ...
                    print(f"[WARNING] Falha ao liberar cache: {cache_e}")
                ...
                break  # Sai do loop em erro diferente
    else:
        print("[FATAL] Máximo de tentativas excedido. Encerrando execução.")